...
func dump(game_state : MMCGameState) -> void:
	...
	if action_dict.is_empty():
		print(_indent(1) + "NO ACTIONS")
		return

	for action in action_dict.keys():
		var tupple : Array = action_dict[action]
		if tupple.is_empty():
			print(_indent(1) + str(action) + " : not evaluated")
		else:
			print(_indent(1) + str(action) + " vvv")
			_dump_internal(2, tupple[0], tupple[1])

func _dump_internal(ind : int, score : MMCScore, game_state : MMCGameState) -> void:
	print(_indent(ind) + str(game_state))
	
	var action_dict : Dictionary = _all_game_states[game_state]
	if action_dict.is_empty():
		print(_indent(ind + 1) + "NO ACTIONS")
		return
		
	for action in action_dict.keys():
		var tupple : Array = action_dict[action]
		if tupple.is_empty():
			print(_indent(ind + 1) + str(action) + " : not evaluated")
		else:
			print(_indent(ind + 1) + str(action) + " vvv")
			_dump_internal(ind + 2, tupple[0], tupple[1])
